...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pos = torch.arange(max_len, dtype=torch.float).unsqueeze(1) #[0,....,max_len-1] will be all i's
        j_index = torch.arange(0, embed_dim, 2, dtype=torch.float) #all even indexes until embed_dim-1
        index_pows=torch.pow(10000,(-1*j_index)/embed_dim)
        pe[0, :, 0::2] = torch.sin(pos * index_pows) #when j is even
        pe[0, :, 1::2] = torch.cos(pos * index_pows) #when j is odd, sub 1 from j by indexing odd's
    ...
    def forward(self, x):
        ...
        pe=self.pe[:,:S,:] #until S cause it's the sequence length
        y=x+pe #like in the notebook, add the input
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        s=torch.matmul(q,k.transpose(2,3))/(math.sqrt(self.head_dim)) #(N,H,S,T)
        
        if attn_mask is not None:
            s = s.masked_fill(attn_mask == 0, float('-inf')) #e^(-inf)~0, so if we want to avoid connection, it will be like that in softmax
        
        w=F.softmax(s,dim=-1) #on T dim 
        w=self.attn_drop(w)#dropout
        out=torch.matmul(w, v) #(N,H,S,T)*(N,H,T,d/h)=(N,H,S,d/h)
        out=out.transpose(1,2).contiguous().view(N,S,E) #(N,H,S,d/h)->(N,S,H,d/h)->reorder in memory->(N,S,E)
        output=self.proj(out) #final MLP
        ############################################################################
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        num_patches_side = self.img_size // self.patch_size #patches num
        x = x.reshape(N, C, num_patches_side, self.patch_size, 
                     num_patches_side, self.patch_size) #(N, C, num_h, ps, num_w, ps)
        x = x.permute(0, 2, 4, 1, 3, 5) #(N, num_h, num_w, C, ps, ps)
        x = x.reshape(N, self.num_patches, self.patch_dim) #(N,num_patches,patch_dim), num_patches=num_h*num_w,patch_dim=ps*ps
        out=self.proj(x)
