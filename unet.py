...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        c_output=self.forward(x, time, model_kwargs)
        u_c_kwargs = model_kwargs
        u_c_kwargs["text_emb"] = None
        u_c_output = self.forward(x, time, u_c_kwargs)
        x=(cfg_scale+1)*c_output-cfg_scale*u_c_output
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        for up_block in self.ups:
            ...
            x = upsample(x)
            # concatenate with skip_net connection
            x = torch.cat([x, skips_net.pop()], dim=1)
            x = resnet1(x, context)
        ...
        #    - Make sure to pass the context to each ResNet block.javascript:void(0)
        ##################################################################
