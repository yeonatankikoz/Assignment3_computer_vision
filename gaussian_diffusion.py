...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        """
        x_start = None
        alphas_cumprod_t=extract(self.sqrt_alphas_cumprod,t,x_t.shape)
        comp_alphas_cumprod_t=extract(self.sqrt_one_minus_alphas_cumprod,t,x_t.shape)
        #x_0=x_t*(1/sqrt(a_bar_t)-(sqrt(1-a_bar_t)/sqrt(a_bar_t)*noise=
        x_start=(1/alphas_cumprod_t)*x_t - (comp_alphas_cumprod_t/alphas_cumprod_t)*noise
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        """
        pred_noise = None
        alphas_cumprod_t=extract(self.sqrt_alphas_cumprod,t,x_start.shape)
        comp_alphas_cumprod_t=extract(self.sqrt_one_minus_alphas_cumprod,t,x_start.shape)
        #noise= x_t/comp_alphas_cumprod_t-(alphas_cumprod_t*x_start)/comp_alphas_cumprod_t
        pred_noise=(1/comp_alphas_cumprod_t)*x_t - (alphas_cumprod_t/comp_alphas_cumprod_t)*x_start
        ####################################################################
        # TODO:
        # Transform x_t and noise to get x_start according to Eq.(4) and Eq.(14).
        # Look at the coeffs in `__init__` method and use the `extract` function.
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        # the model predicts noise, then we use the func predict_start_from_noise 
            x_start = self.predict_start_from_noise(x_t, t, model_output)
        else:  
        # the model predicts x_start directly
            x_start = model_output
        
        x_start = torch.clamp(x_start, -1.0, 1.0) #clamp predicted x_start to the valid range [-1, 1]
        posterior_mean, posterior_std = self.q_posterior(x_start, x_t, t) #q(x_(t-1)|x_t, x_0) mue and sigma
        noise = torch.randn_like(x_t)  #random noise in x_t shape
        mask = (t > 0).float().view(-1, 1, 1, 1) # ensure matrix multiplication dims
        x_tm1 = posterior_mean + posterior_std * noise*mask  #x_(t-1)= mue_t(x_t,x_0)+sigma_t*noise*mask
    ...
    def q_sample(self, x_start, t, noise):
        ...
        alphas_cumprod_t=extract(self.sqrt_alphas_cumprod,t,x_start.shape)
        comp_alphas_cumprod_t=extract(self.sqrt_one_minus_alphas_cumprod,t,x_start.shape)
        x_t=alphas_cumprod_t*x_start+ comp_alphas_cumprod_t*noise   #x_t=sqrt(a_bar_t)*x_0+sqrt(1-a_bar_t)*noise
        ####################################################################
        # TODO:
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        loss = (loss * loss_weight).mean() #weighted mean
