...
def get_similarity_no_loop(text_features, image_features):
    ...
    """
    text_features_mean=text_features/text_features.norm(dim=1, keepdim=True) #mean by sum over D dim, [N,]
    image_features_mean=image_features/image_features.norm(dim=1, keepdim=True) #mean by sum over D dim[M,]
    similarity=torch.matmul(text_features_mean,image_features_mean.T) #[N,M]
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        ).to(device) #two MLP with Relu
    
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=1e-3) #ADAM optimizer
    ...
    def train(self, X_train, Y_train, num_iters=500):
        ...
        for _ in range(num_iters):
            ...
            loss = self.criterion(self.model(X_train), Y_train) #backprop
            loss.backward()
            self.optimizer.step()
    ...
    def inference(self, X_test):
        ...
        pred_classes = torch.argmax(self.model(X_test), dim=1) #hard classifier
        return pred_classes